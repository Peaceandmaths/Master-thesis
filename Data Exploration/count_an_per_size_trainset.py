...
if __name__ == "__main__":
    dataset = "MR"
    all_aneurysms_MR = [] # List to store aggregated metrics
    ...
    aggregated_csv_file_name = f"{dataset}_count_an_per_size_train.csv"
    aggregated_metrics = count_aneurysms_per_size(train_files, aggregated_csv_file_name)
    all_aneurysms_MR.append(aggregated_metrics)
    all_aneurysms_df_MR = pd.concat(all_aneurysms_MR, ignore_index=True)
    all_aneurysms_df_MR.to_csv(f"all_aneurysms_{dataset}_count_an_per_size_train.csv", index=False)
    ...
    aggregated_csv_file_name = f"{dataset}_count_an_per_size_test.csv"
    aggregated_metrics = count_aneurysms_per_size(test_files, aggregated_csv_file_name)
    all_aneurysms_MR.append(aggregated_metrics)
    all_aneurysms_df_MR = pd.concat(all_aneurysms_MR, ignore_index=True)
    all_aneurysms_df_MR.to_csv(f"all_aneurysms_{dataset}_count_an_per_size_test.csv", index=False)
    print(f"Processed {dataset} test ")

    
    # Plot for MR
    # Load the aggregated data for training and internal test sets
    train_df = pd.read_csv('all_aneurysms_MR_count_an_per_size_train.csv')
    test_df = pd.read_csv('all_aneurysms_MR_count_an_per_size_test.csv')
    ...
    plt.plot(merged_df['Size Category'], merged_df['Percentage_Train'], label='Internal Train', linestyle='-', marker='o')
    plt.plot(merged_df['Size Category'], merged_df['Percentage_Test'], label='Internal Test', linestyle='--', marker='x')
    ...
    dataset = "CT"
    all_aneurysms_CT = [] # List to store aggregated metrics
    train_files_path = f'/data/golubeka/nnUNet_Frame/nnUNet_data/nnUNet_raw/Dataset060_IA/labelsTr'
    train_files = [os.path.join(train_files_path, f) for f in os.listdir(train_files_path) if f.endswith('.nii.gz')]
    aggregated_csv_file_name = f"{dataset}_count_an_per_size_train.csv"
    aggregated_metrics = count_aneurysms_per_size(train_files, aggregated_csv_file_name)
    all_aneurysms_CT.append(aggregated_metrics)
    all_aneurysms_df_CT= pd.concat(all_aneurysms_CT, ignore_index=True)
    all_aneurysms_df_CT.to_csv(f"all_aneurysms_{dataset}_count_an_per_size_train.csv", index=False)
    print(f"Processed {dataset} train ")
    ...
    all_aneurysms_CT.append(aggregated_metrics)
    all_aneurysms_df_CT = pd.concat(all_aneurysms_CT, ignore_index=True)
    all_aneurysms_df_CT.to_csv(f"all_aneurysms_{dataset}_count_an_per_size_test.csv", index=False)
    print(f"Processed {dataset} test ")
    
    
    # Plot for CT 
    # Load the aggregated data for training and internal test sets
    train_df = pd.read_csv('all_aneurysms_CT_count_an_per_size_train.csv')
    test_df = pd.read_csv('all_aneurysms_CT_count_an_per_size_test.csv')
